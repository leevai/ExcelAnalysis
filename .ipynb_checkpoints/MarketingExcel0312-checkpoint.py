...
a_data_dict = {row[0]: row[1:] for row in worksheet_a.iter_rows(min_row=2, values_only=True)}
...
for row in worksheet_b.iter_rows(min_row=2):
    ...
    key = row[0].value
    b_data_styles_dict[key] = row_data[1:]
print(f"完成获取:" + config_path + "路径下excelConfig中Excel文件获取")
...
for row_index, row_a in enumerate(worksheet_a.iter_rows(min_row=2, values_only=True)):
    ...
    if id_value in b_data_styles_dict:
        ...
        print(f"数据异常:发现空的项目编号:" + str(id_value))

for row_index, row_b in enumerate(worksheet_b.iter_rows(min_row=2)):
    id_value_b = row_b[0]
    if id_value_b.value not in a_data_dict:
        # 在worksheet_a中新增条目
